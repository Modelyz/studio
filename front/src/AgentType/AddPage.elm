                    ...
                    , Effect.batch
                        [ Shared.dispatchMany s
                            (Message.AddedAgentType at
                                ...
                                ++ List.map (\g -> Message.Grouped (Groupable.AT at) g) (Set.toList model.groups)
                            )
...
validate m =
    Ok <| AgentType (Type.HType HType.AgentType) m.uuid (Maybe.map .uuid m.flatselect) Nothing
