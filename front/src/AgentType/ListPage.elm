                ...
                [ wrappedRow
                    ...
                    [ table [ width fill, Background.color color.table.inner.background ]
                        { data =
                            s.state.agentTypes
                                |> Dict.values
                                |> List.map (\h -> withIdentifiers s.state.identifiers (Type.HType HType.AgentType) h.uuid h)
                                |> List.map (\h -> withGroups s.state.grouped h)
                        , columns =
                            (s.state.identifierTypes
                                |> Dict.values
                                |> List.filter (\it -> Scope.containsScope s.state.agents s.state.agentTypes it.applyTo (HasType (Type.HType HType.AgentType)))
...
groupsColumn : Shared.Model -> Column AgentType msg
groupsColumn s =
