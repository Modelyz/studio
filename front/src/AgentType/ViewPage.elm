...
init s f =
    let
        magentType =
            f.uuid |> Maybe.andThen (State.find s.state.agentTypes)
    in
    ( { route = f.route
      , agentType = f.uuid |> Maybe.andThen (State.find s.state.agentTypes)
      , groups =
            magentType
                |> Maybe.map
                    (\agent ->
        ...
        |> Maybe.map
            (\h ->
                floatingContainer s
                    ...
                    [ h2 "Parent type:"
                    , h.parent
                        |> Maybe.andThen (State.find s.state.agentTypes)
