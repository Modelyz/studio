...
match route =
    case route of
        Route.Entity Route.Process (Route.View uuid tuuid) ->
...
update s msg model =
    case msg of
        Close ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.Process <| Route.List (Maybe.map Uuid.toString model.type_) )
