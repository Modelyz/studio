...
match route =
    case route of
        Route.Entity Route.Process (Route.List tuuid) ->
...
update s msg model =
    case msg of
        ...
        View uuid ->
            ( model, Route.redirect s.navkey (Route.Entity Route.Process (Route.View (Uuid.toString uuid) (Maybe.map Uuid.toString model.filter))) |> Effect.fromCmd )
                    ...
                    [ table [ width fill, Background.color color.table.inner.background ]
                        ...
                        , columns =
                            (s.state.identifierTypes
                                |> Dict.values
                                |> List.filter (\it -> containsScope s.state.types it.scope (HasType (Type.TType TType.Process)))
