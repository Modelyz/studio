...
match route =
    case route of
        Route.Entity Route.Process (Route.List p) ->
...
update s msg model =
    case msg of
        ...
        View uuid ->
            ( model, Route.redirect s.navkey (Route.Entity Route.Process (Route.View { uuid = Uuid.toString uuid, type_ = Maybe.map Uuid.toString model.filter })) |> Effect.fromCmd )
    ...
    case model.viewtype of
        ...
        ViewType.Table ->
            flatContainer s
                ...
                [ wrappedRow
                    [ spacing 10 ]
                    [ tView s.state (HasType (Type.TType TType.Process)) entities
                    ]
                ]
