...
match route =
    case route of
        Route.Entity Route.Contract (Route.View p) ->
...
update s msg model =
    case msg of
        Close ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.Contract <| Route.List {type_ = (Maybe.map Uuid.toString model.type_)} )

        Edit ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.Contract <| Route.Edit {uuid = Uuid.toString model.uuid, type_ = Maybe.map Uuid.toString model.type_} )
...
viewContent model s =
    floatingContainer s
        ...
        , getValues s.state.types s.state.valueTypes s.state.values model.what model.uuid model.type_ False
            |> displayValueDict s { context = ( Type.TType TType.Contract, model.uuid ) } "(none)" s.state.values
        , h2 "Groups:"
        , model.groups
