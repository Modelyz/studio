...
update s msg model =
    case msg of
        Close ->
            ( model, Effect.fromCmd <| redirect s.navkey Route.ContractList )

        Edit ->
            model.contract
                |> Maybe.map
                    (\at ->
                        ( model, Effect.fromCmd <| redirect s.navkey (Route.ContractEdit (Uuid.toString at.uuid)) )
                    )
        ...
        |> Maybe.map
            (\t ->
                floatingContainer s
                    ...
                    , h2 "Values:"
                    , t
                        |> withValues s.state.contracts s.state.contractTypes s.state.valueTypes s.state.values
                        |> .values
