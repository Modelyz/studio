...
match route =
    case route of
        Route.Entity Route.Contract (Route.List p) ->
...
update s msg model =
    case msg of
        ...
        View uuid ->
            ( model, Route.redirect s.navkey (Route.Entity Route.Contract (Route.View { uuid = Uuid.toString uuid, type_ = Maybe.map Uuid.toString model.filter })) |> Effect.fromCmd )
