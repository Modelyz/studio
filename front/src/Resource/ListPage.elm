...
match route =
    case route of
        Route.Entity Route.Resource (Route.List p) ->
...
update s msg model =
    case msg of
        ...
        View uuid ->
            ( model, Route.redirect s.navkey (Route.Entity Route.Resource (Route.View { uuid = Uuid.toString uuid, type_ = Maybe.map Uuid.toString model.filter })) |> Effect.fromCmd )
