...
match route =
    case route of
        Route.Entity Route.EventType (Route.List _) ->
            Just { route = route }
...
update s msg model =
    case msg of
        ...
        View uuid ->
            ( model, Route.redirect s.navkey (Route.Entity Route.EventType (Route.View (Uuid.toString uuid))) |> Effect.fromCmd )
...
viewContent model s =
    let
        allH =
            s.state.eventTypes
                |> Dict.map (\_ v -> hWithIdentifiers s.state.events s.state.eventTypes s.state.identifierTypes s.state.identifiers v)
    in
        ...
        Smallcard ->
            flatContainer s
                ...
                [ wrappedRow
                    ...
                    (allH
                        |> Dict.values
                        |> List.map (\h -> hClickableRemovableCard (View h.uuid) (Removed h.uuid) s.state.events allH s.state.configs h)
                    ...
                    [ table [ width fill, Background.color color.table.inner.background ]
                        ...
                        , columns =
                            (s.state.identifierTypes
                                |> Dict.values
                                |> List.filter (\it -> Scope.containsScope s.state.events s.state.eventTypes it.scope (HasType (Type.HType HType.EventType)))
