        ...
        Smallcard ->
            ...
            flatContainer s
                ...
                [ wrappedRow
                    ...
                    (allHwithIdentifiers
                        |> Dict.values
                        |> List.map (\h -> hClickableRemovableCard (View h.uuid) (Removed h.uuid) s.state.events allHwithIdentifiers s.state.configs h)
            ...
            flatContainer s
                ...
                [ wrappedRow
                    ...
                    [ table [ width fill, Background.color color.table.inner.background ]
                        ...
                        , columns =
                            List.map tableColumn <| List.filter (\it -> Scope.containsScope s.state.events s.state.eventTypes it.applyTo (HasType (Type.HType HType.EventType))) <| Dict.values s.state.identifierTypes
                        }
                    ]
                ]
