    ...
    case msg of
        Edit ->
            model.eventType
                |> Maybe.map
                    (\at ->
                        ( model, Effect.fromCmd <| redirect s.navkey (Route.EventTypeEdit (Uuid.toString at.uuid)) )
                    )
                ...
                let
                    mconfig =
                        model.eventType
                            ...
                            |> Maybe.andThen
                                (\uuid ->
                                    Config.getMostSpecific s.state.events s.state.eventTypes s.state.configs SmallcardTitle (HasUserType (Type.HType HType.EventType) uuid)
                                )
                in
                floatingContainer s
                    "EventType"
