...
update s msg model =
    ...
    case msg of
        InputType met ->
            ( { model
                | flatselect = met
                , identifiers = initIdentifiers s.state.events s.state.eventTypes s.state.identifierTypes (Type.HType HType.EventType) met newUuid
                , uuid = newUuid
                , seed = newSeed
              }
            , Effect.none
                    ...
                    , Effect.batch
                        [ Shared.dispatchMany s
                            (Message.AddedEventType et
                                ...
                                ++ List.map (\g -> Message.Grouped (Groupable.ET et) g) (Set.toList model.groups)
                            )
...
validate m =
    Ok <| EventType (Type.HType HType.EventType) m.uuid (Maybe.map .uuid m.flatselect) Nothing
