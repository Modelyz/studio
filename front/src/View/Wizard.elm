...
view m vs c =
    ...
    if False then
        ...
        in
        column [ width fill, alignTop, padding 20 ]
            [ column [ width fill, Border.shadow shadowStyle, padding 20, centerX, alignTop, spacing 20 ]
                [ Maybe.map
                    (\step ->
                        List.take step c.steps
                            ...
                            |> List.append [ h3 c.context ]
                            |> column [ width fill, alignTop, spacing 20, padding 20 ]
                    )
                    form.step
                    |> Maybe.withDefault (column [ spacing 20 ] (vs ++ [ button.primary { onPress = Just c.open, label = text c.title } ]))
                ]
            ]
