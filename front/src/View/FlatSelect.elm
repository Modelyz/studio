...
flatSelect s c uuids =
    column [ alignTop, spacing 10, width <| minimum 200 fill ]
        [ wrappedRow [ width <| minimum 50 shrink, height (px 48), Border.width 2, padding 3, spacing 4, Border.color color.item.border ] <|
            [ h2 c.title
            , c.muuid
                |> Maybe.map (\uuid -> viewHalfCard s.state (Just <| c.onInput Nothing) s.state.types s.state.configs s.state.identifiers s.state.grouped s.state.groups c.what uuid)
        ...
        , c.muuid
            ...
            |> Maybe.withDefault
                (wrappedRow [ padding 10, spacing 10 ]
                    (uuids
                        |> Dict.values
                        |> List.map (\uuid -> tClickableCard s.state (c.onInput (Just uuid)) s.state.types s.state.configs s.state.identifiers s.state.grouped s.state.groups c.what uuid)
