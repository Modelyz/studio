...
match route =
    case route of
        Route.Entity Route.ContractType (Route.View uuid) ->
...
init s f =
    let
        mcontractType =
            f.uuid |> Maybe.andThen (State.find s.state.contractTypes)
    in
    ( { route = f.route
      , contractType = f.uuid |> Maybe.andThen (State.find s.state.contractTypes)
      , groups =
            mcontractType
                |> Maybe.map
                    (\contract ->
...
update s msg model =
    case msg of
        Close ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.ContractType <| Route.List Nothing )

        Edit ->
            model.contractType
                |> Maybe.map
                    (\h ->
                        ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.ContractType <| Route.Edit (Uuid.toString h.uuid) )
                    )
        ...
        |> Maybe.map
            (\h ->
                floatingContainer s
                    ...
                    [ h2 "Parent type:"
                    , h.parent
                        |> Maybe.andThen (State.find s.state.contractTypes)
                    ...
                    , h2 "Values:"
                    , h
                        |> hWithValues s.state.contracts s.state.contractTypes s.state.valueTypes s.state.values
                        |> .values
