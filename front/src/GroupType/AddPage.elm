...
update s msg model =
    ...
    case msg of
        InputType mgt ->
            ( { model
                | flatselect = mgt
                , identifiers = initIdentifiers s.state.groups s.state.groupTypes s.state.identifierTypes (Type.HType HType.GroupType) mgt newUuid
                , uuid = newUuid
                , seed = newSeed
              }
            , Effect.none
...
validate m =
    Ok <| GroupType (Type.HType HType.GroupType) m.uuid (Maybe.map .uuid m.flatselect)
