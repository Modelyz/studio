...
match route =
    case route of
        Route.Entity Route.ProcessType (Route.View uuid _) ->
...
update s msg model =
    case msg of
        Close ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.ProcessType <| Route.List (Maybe.map Uuid.toString model.type_) )

        Edit ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.ProcessType <| Route.Edit (Uuid.toString model.uuid) Nothing )
