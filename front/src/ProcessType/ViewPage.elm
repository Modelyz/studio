...
init s f =
    let
        mprocessType =
            f.uuid |> Maybe.andThen (State.find s.state.processTypes)
    in
    ( { route = f.route
      , processType = f.uuid |> Maybe.andThen (State.find s.state.processTypes)
      , groups =
            mprocessType
                |> Maybe.map
                    (\process ->
    ...
    case msg of
        ...
        Edit ->
            model.processType
                |> Maybe.map
                    (\h ->
                        ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.ProcessType <| Route.Edit (Uuid.toString h.uuid) )
                    )
        ...
        |> Maybe.map
            (\h ->
                floatingContainer s
                    ...
                    [ h2 "Parent type:"
                    , h.parent
                        |> Maybe.andThen (State.find s.state.processTypes)
                    ...
                    , h2 "Values:"
                    , h
                        |> hWithValues s.state.processes s.state.processTypes s.state.valueTypes s.state.values
                        |> .values
