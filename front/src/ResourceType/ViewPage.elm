...
match route =
    case route of
        Route.Entity Route.ResourceType (Route.View uuid) ->
...
init s f =
    let
        mresourceType =
            f.uuid |> Maybe.andThen (State.find s.state.resourceTypes)
    in
    ( { route = f.route
      , resourceType = f.uuid |> Maybe.andThen (State.find s.state.resourceTypes)
      , groups =
            mresourceType
                |> Maybe.map
                    (\resource ->
...
update s msg model =
    case msg of
        Close ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.ResourceType <| Route.List Nothing )

        Edit ->
            model.resourceType
                |> Maybe.map
                    (\h ->
                        ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.ResourceType <| Route.Edit (Uuid.toString h.uuid) )
                    )
        ...
        |> Maybe.map
            (\h ->
                floatingContainer s
                    ...
                    [ h2 "Parent type:"
                    , h.parent
                        |> Maybe.andThen (State.find s.state.resourceTypes)
                    ...
                    , h2 "Values:"
                    , h
                        |> hWithValues s.state.resources s.state.resourceTypes s.state.valueTypes s.state.values
                        |> .values
