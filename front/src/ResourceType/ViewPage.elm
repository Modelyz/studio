module ResourceType.ViewPage exposing (..)
    ...
    case msg of
        Edit ->
            model.resourceType
                |> Maybe.map
                    (\at ->
                        ( model, Effect.fromCmd <| redirect s.navkey (Route.ResourceTypeEdit (Uuid.toString at.uuid)) )
                    )
                ...
                let
                    mconfig =
                        model.resourceType
                            ...
                            |> Maybe.andThen
                                (\uuid ->
                                    Config.getMostSpecific s.state.resources s.state.resourceTypes s.state.configs SmallcardTitle (HasUserType (Type.HType HType.ResourceType) uuid)
                                )
                in
                floatingContainer s
                    "ResourceType"
                    ...
                    , at.parent
                        ...
                        |> Maybe.map (Identifiable.display mconfig)
