...
init s f =
    ...
    in
    ( f.uuid
        |> Maybe.andThen (T.find s.state.events)
        |> Maybe.map
            (\a ->
                let
                    oldGroups =
                ...
                in
                { route = f.route
                , flatselect = H.find s.state.eventTypes a.type_
                , uuid = a.uuid
                , seed = newSeed
                , identifiers =
                    initIdentifiers s.state.events s.state.eventTypes s.state.identifierTypes (Type.TType TType.Event) Nothing a.uuid
...
update s msg model =
    case msg of
        InputType mat ->
            ( { model
                | flatselect = mat
                , identifiers = initIdentifiers s.state.events s.state.eventTypes s.state.identifierTypes (Type.TType TType.Event) mat model.uuid
              }
            , Effect.none
            )
                ...
                Ok a ->
                    ...
                    , Effect.batch
                        [ Shared.dispatchMany s
                            (Message.AddedEvent a
                                :: List.map Message.IdentifierAdded (Dict.values model.identifiers)
                                ++ List.map (\g -> Message.Grouped (Groupable.E a) g) (Dict.values addedGroups)
                                ++ List.map (\g -> Message.Ungrouped (Groupable.E a) g) (Dict.values removedGroups)
                            )
                ...
                Step.Step StepType ->
                    ...
                    column [ alignTop, spacing 10, width <| minimum 200 fill ]
                        [ wrappedRow [ width <| minimum 50 shrink, Border.width 2, padding 3, spacing 4, Border.color color.item.border ] <|
                            [ h2 "Type"
                            , model.flatselect
                                |> Maybe.map (withIdentifiers s.state.events s.state.eventTypes s.state.identifierTypes s.state.identifiers)
