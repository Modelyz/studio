...
init s f =
    ...
    in
    ( f.uuid
        |> Maybe.andThen (T.find s.state.agents)
        |> Maybe.map
            (\a ->
                let
                    oldGroups =
                ...
                in
                { route = f.route
                , flatselect = H.find s.state.agentTypes a.type_
                , uuid = a.uuid
                , seed = newSeed
                , identifiers =
                    initIdentifiers s.state.agents s.state.agentTypes s.state.identifierTypes (Type.TType TType.Agent) Nothing a.uuid
...
update s msg model =
    case msg of
        InputType mat ->
            ( { model
                | flatselect = mat
                , identifiers = initIdentifiers s.state.agents s.state.agentTypes s.state.identifierTypes (Type.TType TType.Agent) mat model.uuid
              }
            , Effect.none
            )
                ...
                Ok a ->
                    ...
                    , Effect.batch
                        [ Shared.dispatchMany s
                            (Message.AddedAgent a
                                :: List.map Message.IdentifierAdded (Dict.values model.identifiers)
                                ++ List.map (\g -> Message.Grouped (Groupable.A a) g) (Dict.values addedGroups)
                                ++ List.map (\g -> Message.Ungrouped (Groupable.A a) g) (Dict.values removedGroups)
                            )
...
checkStep model =
    case model.step of
        Step StepType ->
            Maybe.map (\_ -> Ok ()) model.flatselect |> Maybe.withDefault (Err "You must select an Agent Type")
...
validate m =
    case m.flatselect of
        Just at ->
            ...
            Ok <| Agent (Type.TType TType.Agent) m.uuid at.uuid Dict.empty Dict.empty Dict.empty Dict.empty
