...
match route =
    case route of
        Route.Entity Route.Agent (Route.List _) ->
            Just { route = route }
...
update s msg model =
    case msg of
        ...
        View uuid ->
            ( model, Route.redirect s.navkey (Route.Entity Route.Agent (Route.View (Uuid.toString uuid))) |> Effect.fromCmd )
                    ...
                    [ table [ width fill, Background.color color.table.inner.background ]
                        ...
                        , columns =
                            (s.state.identifierTypes
                                |> Dict.values
                                |> List.filter (\it -> Scope.containsScope s.state.agents s.state.agentTypes it.scope (HasType (Type.TType TType.Agent)))
