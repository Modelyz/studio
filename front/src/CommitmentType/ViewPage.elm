...
init s f =
    let
        mcommitmentType =
            f.uuid |> Maybe.andThen (State.find s.state.commitmentTypes)
    in
    ( { route = f.route
      , commitmentType = f.uuid |> Maybe.andThen (State.find s.state.commitmentTypes)
      , groups =
            mcommitmentType
                |> Maybe.map
                    (\commitment ->
    ...
    case msg of
        ...
        Edit ->
            model.commitmentType
                |> Maybe.map
                    (\h ->
                        ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.CommitmentType <| Route.Edit (Uuid.toString h.uuid) )
                    )
        ...
        |> Maybe.map
            (\h ->
                floatingContainer s
                    ...
                    [ h2 "Parent type:"
                    , h.parent
                        |> Maybe.andThen (State.find s.state.commitmentTypes)
                        |> Maybe.map (hWithIdentifiers s.state.commitments s.state.commitmentTypes s.state.identifierTypes s.state.identifiers)
                    ...
                    , h2 "Identifiers:"
                    , h
                        |> hWithIdentifiers s.state.commitments s.state.commitmentTypes s.state.identifierTypes s.state.identifiers
                        |> .identifiers
