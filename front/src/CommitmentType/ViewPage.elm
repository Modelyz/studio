...
match route =
    case route of
        Route.Entity Route.CommitmentType (Route.View uuid _) ->
...
update s msg model =
    case msg of
        Close ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.CommitmentType <| Route.List (Maybe.map Uuid.toString model.type_) )

        Edit ->
            ( model, Effect.fromCmd <| redirect s.navkey <| Route.Entity Route.CommitmentType <| Route.Edit (Uuid.toString model.uuid) Nothing )
...
viewContent model s =
    floatingContainer s
        ...
        , h2 "Default quantity"
        , model.ct |> Maybe.map .qty |> Maybe.map (Expression.View.inputExpression s { context = ( Type.HType HType.CommitmentType, model.uuid ) }) |> Maybe.withDefault (text "(none)")
        , h2 "Restrictions:"
