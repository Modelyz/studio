...
init s f =
    ...
    ( { route = f.route
      ...
      , uuid = newUuid
      , seed = newSeed
      , identifiers = initIdentifiers s.state.commitments s.state.commitmentTypes s.state.identifierTypes (Type.HType HType.CommitmentType) Nothing newUuid
      , groups = Dict.empty
      , warning = ""
...
update s msg model =
    case msg of
        InputType mcmt ->
            ( { model
                | flatselect = mcmt
                , identifiers = initIdentifiers s.state.commitments s.state.commitmentTypes s.state.identifierTypes (Type.HType HType.CommitmentType) mcmt model.uuid
              }
            , Effect.none
            )
                ...
                Ok cmt ->
                    ...
                    , Effect.batch
                        [ Shared.dispatchMany s
                            (Message.AddedCommitmentType cmt
                                :: List.map Message.IdentifierAdded (Dict.values model.identifiers)
                                ++ List.map (\g -> Message.Grouped (Groupable.CmT cmt) g) (Dict.values model.groups)
                            )
...
validate m =
    Ok <| CommitmentType (Type.HType HType.CommitmentType) m.uuid (Maybe.map .uuid m.flatselect) Dict.empty
        ...
        step =
            case model.step of
                ...
                Step.Step StepIdentifiers ->
                    let
                        scope =
                            model.flatselect |> Maybe.map (\h -> HasUserType (Type.HType HType.CommitmentType) h.uuid) |> Maybe.withDefault (HasType (Type.HType HType.CommitmentType))
                    in
                    inputIdentifiers { onEnter = Added, onInput = InputIdentifier } model scope
    in
    floatingContainer s
        "Adding an CommitmentType"
