...
init s f =
    ...
    ( f.uuid
        ...
        |> Maybe.map
            (\at ->
                { route = f.route
                , flatselect = at.parent |> Maybe.andThen (H.find s.state.commitmentTypes)
                , uuid = at.uuid
                , seed = newSeed
                , identifiers =
...
update s msg model =
    case msg of
        InputType mat ->
            ( { model
                | flatselect = mat
                , identifiers = initIdentifiers s.state.commitments s.state.commitmentTypes s.state.identifierTypes (Type.HType HType.CommitmentType) mat model.uuid
              }
            , Effect.none
            )
