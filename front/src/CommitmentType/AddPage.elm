...
update s msg model =
    ...
    case msg of
        InputType mrt ->
            ( { model
                | flatselect = mrt
                , identifiers = initIdentifiers s.state.commitments s.state.commitmentTypes s.state.identifierTypes (Type.HType HType.CommitmentType) mrt newUuid
                , uuid = newUuid
                , seed = newSeed
              }
            , Effect.none
                    ...
                    , Effect.batch
                        [ Shared.dispatchMany s
                            (Message.AddedCommitmentType cmt
                                ...
                                ++ List.map (\g -> Message.Grouped (Groupable.CmT cmt) g) (Set.toList model.groups)
                            )
...
validate m =
    Ok <| CommitmentType (Type.HType HType.CommitmentType) m.uuid (Maybe.map .uuid m.flatselect) Nothing
                    ...
                    column [ alignTop, spacing 10, width <| minimum 200 fill ]
                        ...
                        , wrappedRow [ Border.width 2, padding 10, spacing 10, Border.color color.item.border ] <|
                            List.map
                                ...
                                (Set.toList <| s.state.commitmentTypes)
                        ]
