...
update s msg model =
    case msg of
        ...
        View uuid ->
            ( model, Route.redirect s.navkey (Route.Entity Route.CommitmentType (Route.View (Uuid.toString uuid) Nothing)) |> Effect.fromCmd )
                        ...
                        , columns =
                            Type.View.typeColumn s
                                :: (s.state.identifierTypes
                                        |> Dict.values
                                        |> List.filter (\it -> containsScope s.state.types it.scope (HasType (Type.HType HType.CommitmentType)))
