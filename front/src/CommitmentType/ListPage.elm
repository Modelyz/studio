...
match route =
    case route of
        Route.Entity Route.CommitmentType (Route.List _) ->
            Just { route = route }
...
update s msg model =
    case msg of
        ...
        View uuid ->
            ( model, Route.redirect s.navkey (Route.Entity Route.CommitmentType (Route.View (Uuid.toString uuid))) |> Effect.fromCmd )
    ...
    let
        allH =
            s.state.commitmentTypes
                |> Dict.map (\_ v -> hWithIdentifiers s.state.commitments s.state.commitmentTypes s.state.identifierTypes s.state.identifiers v)
    in
        ...
        Smallcard ->
            flatContainer s
                ...
                [ wrappedRow
                    ...
                    (allH
                        |> Dict.values
                        |> List.map (\h -> hClickableRemovableCard (View h.uuid) (Removed h.uuid) s.state.commitments allH s.state.configs h)
                    ...
                    [ table [ width fill, Background.color color.table.inner.background ]
                        ...
                        , columns =
                            (s.state.identifierTypes
                                |> Dict.values
                                |> List.filter (\it -> Scope.containsScope s.state.commitments s.state.commitmentTypes it.scope (HasType (Type.HType HType.CommitmentType)))
